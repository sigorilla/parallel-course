    ...
    if (rank == 0){
        ...
        for (j = 0; j <= m; j++){
            ...
            if (j == 0){
                ...
                u2[j] = f(x[j],t[i-1])*tau - (u1[j+1] - u1[j-1])*tau/(2*h) + 0.5*(u1[j+1] + u1[j-1]);
                MPI_Send(&u2[j], 1, MPI_DOUBLE, (rank+1)%size, j, MPI_COMM_WORLD);
            } else {
                u2[j] = f(x[j],t[i-1])*tau - (u1[j] - u1[j-1])*tau/h + u1[j];
                for (l = 0; l <= m; l++){
    ...
    for (i; i < k+1; i += size){
        for (j = 0; j <= m; j++){
            if (j == 0){
                u1[j] = b(t[i]);
                u2[j] = b(t[i]);
                MPI_Recv(&u1[1], 1, MPI_DOUBLE, (rank-1+size)%size, j+1, MPI_COMM_WORLD, &status);
            } else if (j < m) {
                MPI_Recv(&u1[j+1], 1, MPI_DOUBLE, (rank-1+size)%size, j+1, MPI_COMM_WORLD, &status);
                u2[j] = f(x[j],t[i-1])*tau - (u1[j+1] - u1[j-1])*tau/(2*h) + 0.5*(u1[j+1] + u1[j-1]);
                if (i != k)
                    MPI_Send(&u2[j], 1, MPI_DOUBLE, (rank+1)%size, j, MPI_COMM_WORLD);
            } else {
